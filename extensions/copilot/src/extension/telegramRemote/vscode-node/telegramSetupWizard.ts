...
export function buildTelegramConsentDetail(workstationLabel: string, workspaceLabel: string, capabilities: TelegramRemoteCapabilities = { remotePermissionResponses: false }): string {
	...
	return l10n.t('A paired Telegram user can send Copilot prompts, steer an active Telegram-started turn, stop Telegram-started work, and browse bounded text previews of files in the authorized workspace. Prompts can cause this machine to write files, run shell commands, access the network, and perform Git operations, subject to Copilot’s normal controls.\n\n{0}\n\nTelegram bot chats are not end-to-end encrypted. Prompts, final answers, workspace file contents and paths, and any activity detail enabled locally transit Telegram infrastructure.\n\nAnyone who obtains the bot token can impersonate the bot. Anyone who obtains the paired Telegram account gains this control.\n\nWorkstation exposed: {1}\nWorkspace exposed: {2}\n\nTo turn it off, select the Telegram status bar item and choose Disable Remote Access.', permissionDetail, workstationLabel, workspaceLabel);
}
